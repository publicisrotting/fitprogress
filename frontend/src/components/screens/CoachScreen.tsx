...
export default function CoachScreen({ onNavigate }: CoachScreenProps) {
  ...
  return (
    <div className="h-full apple-bg overflow-y-auto pb-24 relative overflow-x-hidden">
      ...
      <div className="relative z-10 px-6 pt-[calc(3rem+env(safe-area-inset-top))] pb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="apple-text-2 text-xs font-bold uppercase tracking-widest">{t('coach.subtitle')}</p>
          </div>
      ...
      <div className="relative z-10 px-6 space-y-5">
        ...
        {data?.message === 'complete_first_workout' && (
          <div className="bg-white/5 border border-dashed border-white/10 rounded-[2.5rem] p-10 text-center">
            ...
            <p className="apple-text font-bold mb-2">Почни тренування</p>
            <p className="apple-text-2 text-sm mb-6">Збережи перше тренування з вагами, щоб отримати рекомендації</p>
        ...
        {data && data.message !== 'complete_first_workout' && !data.hasRealData && (
          <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-[2rem] p-6 flex items-start gap-4">
            ...
            <div>
              <p className="apple-text font-bold mb-1">Заплановано {data.plannedWorkouts} тренувань</p>
              <p className="apple-text-2 text-sm leading-relaxed">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white/5 border border-white/5 rounded-[1.8rem] p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[10px] font-black apple-text-3 uppercase tracking-widest">Тренувань</p>
              </div>
            ...
            <div className="bg-white/5 border border-white/5 rounded-[1.8rem] p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[10px] font-black apple-text-3 uppercase tracking-widest">Вправ</p>
              </div>
        ...
        {cfg && recovery && (
          <div className={`${cfg.bg} border ${cfg.border} rounded-[2rem] p-6 flex items-start gap-4`}>
            ...
            <div>
              <p className="text-[10px] font-black apple-text-2 uppercase tracking-widest mb-1">{t('coach.recovery')}</p>
              <p className="apple-text font-bold leading-relaxed">{t(`coach.${cfg.textKey}`)}</p>
              {recovery.days && (
                <p className="text-sm apple-text-2 mt-1">{recovery.days} {t('coach.daysSince')}</p>
              )}
            </div>
          </div>
        )}
        ...
        {data?.deloadRecommended && (
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-[2rem] p-6 flex items-start gap-4">
            ...
            <div>
              <p className="text-[10px] font-black apple-text-2 uppercase tracking-widest mb-1">{t('coach.deload')}</p>
              <p className="apple-text font-bold leading-relaxed">{t('coach.deloadDesc')}</p>
            </div>
          </div>
        )}
        ...
        {(data?.suggestions || []).length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="apple-text-2 text-[10px] font-black uppercase tracking-[0.2em]">{t('coach.overload')}</h3>
            </div>
              ...
              {data!.suggestions.map((s, idx) => (
                <div key={idx} className="bg-white/5 border border-white/5 rounded-[2rem] p-6">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <p className="apple-text font-bold text-sm">{s.exercise}</p>
                      <p className="text-[10px] font-bold apple-text-3 uppercase tracking-widest">Прогресивне навантаження</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white/5 rounded-2xl p-4 text-center border border-white/5">
                      ...
                      <p className="text-[10px] apple-text-3 mt-1">× {s.current.reps} повт</p>
                    </div>
        ...
        {data && data.hasRealData && (data.suggestions || []).length === 0 && !data.deloadRecommended && (
          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-[2rem] p-8 text-center">
            ...
            <p className="apple-text font-bold mb-1">Все чудово!</p>
            <p className="apple-text-2 text-sm">Зроби ще кілька тренувань — тренер проаналізує прогрес</p>
          </div>
        )}
        ...
        {(data?.weeklyVolume || []).filter(d => d.volume > 0).length > 0 && (
          <div className="bg-white/5 border border-white/5 rounded-[2.5rem] p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="apple-text font-bold">{t('coach.weeklyVolumeTitle')}</h3>
            </div>
            <div className="h-48">
