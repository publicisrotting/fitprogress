...
function getResponse(question: string, ctx?: any): string {
  ...
  // Strict topic guard — only sports / fitness / nutrition / recovery
  const FITNESS_VOCAB = [
    'трен','вправ','м\'яз','мьяз','мяз','підхід','подход','повтор','вага','ваги','вес','жим','тяга','присід','присед',
    'кардіо','кардио','біцепс','бицепс','трицепс','спина','груд','ног','плеч','прес','живіт','живот','сіл','сил','масу','маса',
    'схуд','жир','білок','белок','протеїн','протеин','калор','харчув','питан','дієт','диет','їж','еда','креатин','добав',
    'відновл','восстанов','розтяж','растяж','розминк','разминк','сон','спорт','фітнес','фитнес','gym','зал','штанг','гантел',
    'програм','підхід','rep','set','workout','muscle','protein','squat','bench','deadlift','cardio','стрек','streak','крепатур',
    'болят','болить','ниют','делоад','deload','плато','розклад','графік','графiк','скільки','как часто','як часто'
  ];
  const seemsFitness = FITNESS_VOCAB.some(w => q.includes(w));
  if (!seemsFitness) {
    return 'Я — спортивний AI-тренер і відповідаю лише на питання про тренування, техніку вправ, харчування, відновлення та фітнес 🏋️\n\nЦе питання не стосується спорту. Запитай мене, наприклад, про програму тренувань, набір маси чи техніку присідань.';
  }
